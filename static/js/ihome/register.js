...
function sendSMSCode() {
    ...
    var res_data = {mobile:mobile, image_code_text:imageCode, image_code_id:imageCodeId}
    $.ajax({
        url:"/api/smscode",
        type:"post",
...
function register() {
    $(".form-register").submit(function (e) {
        ...
        if (!mobile){
            $("#mobile-err").html("手机号不能为空");
            $("#mobile-err").show();
        }
        if (!phonecode){
            $("#phone-code-err").html("请输入短信验证码");
            $("#phone-code-err").show();
        }
        if (!password){
            $("#password-err").html("请输入密码");
            $("#password-err").show();
        }
        ...
        $.ajax({
            ...
            success:function (rep_data) {
                if ("0" == rep_data.errcode){
                    location.href = "/login.html";
                }else if ("验证码过期" == rep_data.errmsg || "验证码错误" == rep_data.errmsg || "手机号已存在" == rep_data.errmsg) {
                    $("#phone-code-err>span").html(rep_data.errmsg);
                    $("#phone-code-err").show();
                }
